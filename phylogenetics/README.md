...
To be able to use our data in the phylogenetics softwares, we first converted the BAM files to fasta files with the -doFasta function of ANGSD. The command was as follows:
...
-out individual_genome_minD10
```

Most of the options here are explained in [General ANGSD Options](../#general-options). The most important options in here were -doFasta, which specifies how to create the fasta files and setMinDepth, which determines the minimum coverage for a base to be sampled.

### Creating Alignments

We subset the genes based on their position information and extracted them to separate fasta files.
...
The bed file should be tab separated containing the chromosome number, start and end positions of the genes in the first three columns, respectively. Optionally, gene IDs can be added to the fourth column. Most importantly, the bed file is zero-based unlike our position file which is 1-based. That's why we manipulated the position info file for our needs with the following commands.
...
These were the commands to extract only chromosome 1 from the file. The final output from these had redundant characters in the geneID column. However, we just wanted to know the cluster IDs which are gene IDs, so we organized them with the following commands:
...
```
The first deletes the first part of the column 4, before cluster IDs and the second reorders the columns. The third converts the whitespace delimiter to tab separator and creates the final bed file. The same commands were run to convert the position file of coding sequence and it was created a bed file named cdsPosition.bed which is available in the same directory.
...
This command does not extract fasta files for each gene, but rather separates them inside the main fasta file belonging to one individual by giving headers to the genes. The `-name` option puts the information in the bed file (chromosome, start-end, gene ID) into the header.

We changed the header by adding the individual names, so they would be identifiable in the final alignment:
...
To exclude the genes with too much missing data, we needed to calculate the percentages of Ns in each gene and for each individuals. This was done with the following commands:
...
So, at the end we had files for each individual containing the number of Ns and the total length of the genes. The files were imported to R and manipulated there using [stat_md10.R](stat_md10.R). Firstly, we find the percentage of Ns by dividing the number of Ns to the total length and multiplying the values with 100. The data frames for each individual containing the percentages were connected into a matrix. We visualized this matrix as a heatmap for the first 50 genes in Figure S5. We then created a list of genes which are >50% complete in all individuals and a list of genes not present in all individuals, but in where at least 70% of individuals have >50% completeness. We calculated this using [selection_10.R](selection_10.R) which uses the matrix from stat_md10.R to produce the lists of genes and which individuals should be included for them.
...
The command takes the gene IDs with no missing individuals from the none_geneIDs_md10 file created in the filtering step and reads in each fasta file for every individual and creates an output named with the gene ID. The loop stops after finishing all genes in the file. For the genes with missing individuals, the missing individuals varied from gene to gene, so after filtering, we created lists for each gene containing the individual IDs sequences for the gene. By making use of these files, gene alignments were created for the genes containing missing individuals with the following command:
...
The commands read the gene IDs from the files, go into the directories with the same name and grep the defined characters inside the iqtree file with the value following the sentence. The commands after the grep is for getting the values alone, without the whole sentence and these values were directed to an output. The output files had the same order with the gene ID files, thus we know specifically which value belongs to which gene tree. We imported the files containing information of the log-likelihoods, quartet resolutions, and RF distances into R and visualized them as density plots, ternary plots and box plots, respectively.
...
Each group is determined with one line, following the IDs of the individuals that will be clustered in it. After preparation of these files, we ran ASTRAL with the heuristic version (default) at the individual and population levels for each technical replicate. Species level was run only for the first replicate. The general commands used were:
...
- EN: Effective number of genes. If there is missing data, some gene trees will have nothing to say about a branch and EN will be smaller for that branch.
