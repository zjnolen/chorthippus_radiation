...
def Optimize_Routine(fs, pts, outfile, model_name, func, rounds, param_number, fs_folded=True, reps=None, maxiters=None, folds=None, in_params=None, in_upper=None, in_lower=None, param_labels=" "):
    ...
    for r in range(rounds):
        ...
        for rep in range(1, (reps_list[r]+1) ):
			...
			#ZN: Updated the output file name to the new templogfile name and changed from optimize_log_fmin to optimize_log
            params_opt = dadi.Inference.optimize_log(params_perturbed, fs, func_exec, pts, lower_bound=lower_bound, upper_bound=upper_bound, verbose=1, maxiter=maxiters_list[r], output_file = "temp/{0}.{1}.log.txt".format(outfile, model_name))
